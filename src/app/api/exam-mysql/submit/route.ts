...
export async function POST(request: NextRequest) {
  try {
    ...
    // 更新会话
    await mysqlDatabase.updateSession(sessionId, {
      answers,
      score,
      endTime,
      status: 'completed',
    ...
    // 获取更新后的会话（带题目信息）
    const updatedSessionWithQuestions = await mysqlDatabase.getSessionWithQuestions(sessionId);
    ...
    console.error('MySQL提交答案时发生错误:', error);
