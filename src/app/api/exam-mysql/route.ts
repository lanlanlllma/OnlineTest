...
export async function POST(request: NextRequest) {
  try {
    ...
    // 获取题目
    const questions = await mysqlDatabase.getRandomQuestions(totalQuestions, category, difficulty);
    ...
    const sessionId = await mysqlDatabase.createSession(sessionData);
    ...
    console.error('创建MySQL考试会话时发生错误:', error);
...
export async function GET(request: NextRequest) {
  try {
    ...
    const sessionWithQuestions = await mysqlDatabase.getSessionWithQuestions(sessionId);
    ...
    console.error('获取MySQL考试会话时发生错误:', error);
